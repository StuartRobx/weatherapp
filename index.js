...
function weatherReport(latitude, longitude) {
	...
	$.getJSON(api_call, function(forecast) {
		...
		$( "#forecast" ).replaceWith( $( '<ul class="list-reset fadein-stagger" id="forecast"></ul>' ) );
		...
		for(var i = 0, l = forecast.daily.data.length; i < l - 1; i++) {
			...
			$("#forecast").append(
					...
					"<div class='date'><strong>" + moment(date).format('ddd ') + "</strong>" + moment(date).format('MMM Do') + "</div>" +
					"<div class='graphic'><canvas class=" + skicons + "></canvas></div>" +
					"<div class='forecasttemp'>" + temp + "&deg;</div>" +
					"<!--<div><b>High</b>: " + tempMax + "&deg;</div>-->" +
...
$('button').on('click', function(e) {
	...
	if(lat && long !== '') {
		...
		$('.form').fadeOut(100, function() {
			...
			$('.screen').append('<button id="back">New Forecast</button><h3 class="city">' + city_name + '</h3><div id="forecast"><div id="loading"><img src="./img/loading.gif" alt="loading" /></div></div>');
		});
	}
});
